" Vim indent file
" Language:     OCaml
" Maintainers:  Raphaël Proust      <raphlalou@gmail.com>
...
let s:letpat = '^\s*\(let\|lwt\|type\|module\|class\|open\|exception\|val\|include\|external\)\>'
let s:letlim = '\(\<\(sig\|struct\)\|;;\)\s*$'
let s:lim = '^\s*\(exception\|external\|include\|let\|lwt\|module\|open\|type\|val\)\>'
let s:module = '\<\%(begin\|sig\|struct\|object\)\>'
let s:obj = '^\s*\(constraint\|inherit\|initializer\|method\|val\)\>\|\<\(object\|object\s*(.*)\)\s*$'
let s:type = '^\s*\%(class\|let\|lwt\|type\)\>.*='
...
function s:FindLet(pstart, pmid, pend)
 call search(a:pend, 'bW')
 return indent(searchpair(a:pstart, a:pmid, a:pend, 'bWn', 'synIDattr(synID(line("."), col("."), 0), "name") =~? "string\\|comment" || getline(".") =~ "^\\s*\\(let\\|lwt\\)\\>.*=.*\\<in\\s*$" || getline(prevnonblank(".") - 1) =~ s:beflet'))
endfunction
...
function GetOCamlIndent()
 ...
 elseif line =~ '^\s*\(let\|lwt\)\>'
   if lline !~ s:lim . '\|' . s:letlim . '\|' . s:beflet
     return s:FindLet(s:type, '','\<\(let\|lwt\)\s*$')
   endif
 ...
 elseif line =~ '^\s*|'
   if lline !~ '^\s*\(|[^\]]\|\(match\|match_lwt\|type\|with\)\>\)\|\<\(function\|parser\|private\|with\)\s*$'
     call search('|', 'bW')
     return indent(searchpair('^\s*\(match\|match_lwt\|type\)\>\|\<\(function\|parser\|private\|with\)\s*$', '', '^\s*|', 'bWn', 'synIDattr(synID(line("."), col("."), 0), "name") =~? "string\\|comment" || getline(".") !~ "^\\s*|.*->"'))
   endif
 ...
 elseif line =~ '^\s*in\>'
   if lline !~ '^\s*\(let\|lwt\|and\)\>'
     return s:FindPair('\<let\|lwt\>', '', '\<in\>')
   endif
 ...
 elseif line =~ '^\s*and\>'
   if lline !~ '^\s*\(and\|let\|lwt\|type\)\>\|\<end\s*$'
     return ind - &sw
   endif

 " Indent if current line begins with 'with':
 elseif line =~ '^\s*with\>'
   if lline !~ '^\s*\(match\|match_lwt\|try\)\>'
     return s:FindPair('\<\%(match\|match_lwt\|try\)\>', '','\<with\>')
   endif
 ...
 if lline =~ '\<\(match\|match_lwt\)\>.*\<with\>\s*\<parser\s*$'
   let ind = ind - &sw
 endif
